...
# final_df = pd_centers(columns, centers)
# print(final_df.to_string())

# print(clusters)
df1 = pd.DataFrame(clusters, columns=['cluster'])
df1.index.name = 'userId'
df1.index += 1

grouped = df1.groupby('cluster')

merged_df = pd.merge(df_rat, df_mov, on="movieId", how="outer")
...
def user_titles_ratings_func():
    for cluster_num, group in grouped:
        ...
        for user_id in users_in_cluster:
            ...
            user_unrated_movies = titles_ratings[titles_ratings.isnull()]  # Which movies did they not rate? -> series
